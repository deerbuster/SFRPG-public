...
function convertSpellShortDescToString(nodeSpell)
	local nodeDesc = nodeSpell.getChild("summary");
	if nodeDesc then
		local sDescType = nodeDesc.getType();
		if sDescType == "formattedtext" then
			local sDesc = nodeDesc.getText();
			...
			nodeDesc.delete();
			DB.setValue(nodeSpell, "shortdescription", "string", sDesc);
		end
	end
end
...
function addSpell(nodeSource, nodeSpellClass, nLevel)
	...
	convertSpellShortDescToString(nodeNewSpell);
	-- Convert the description field from module data
	convertSpellDescToString(nodeNewSpell);
	--Update Spell Save field
	sSave = DB.getValue(nodeNewSpell, "save","");
	sSaveSource = DB.getValue(nodeNewSpell, "savingthrow","");
	if sSave == "" then
		if sSaveSource ~= "" then
			DB.setValue(nodeNewSpell, "save", "string", sSaveSource);
		end
	end
...
function addSpellCastAction(nodeSpell)
	...
	if not sSave:match("harmless") then
		...
		if sSave:match("half") then
			...
			local bHalf = parseSpellSave(nodeSpell);
			if bHalf then
				DB.setValue(nodeAction, "onmissdamage", "string", "half");
			end
		end
...
function parseSpellSave(nodeSpell)
...
function parseSpell(nodeSpell)
	...
	local aDamages = {};
	local aHeals = {};
	local sLevel = DB.getValue(nodeSpell, "level", 0);
	local i = 1;
	while aWords[i] do
		-- Main trigger ("damage")
		if StringManager.isWord(aWords[i], "damage") then
...
function getSpellAction(rActor, nodeAction, sSubRoll)
	...
	elseif sType == "effect" then
		...
		rAction.sName = EffectManagerSFRPG.evalEffect(rActor, DB.getValue(nodeAction, "label", ""), nodeSpellClass);
