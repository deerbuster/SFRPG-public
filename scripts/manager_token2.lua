...
function onInit()
	TokenManager.addDefaultHealthFeatures(getHealthInfo, {"hp","sp", "hptemp", "fatique", "wounds"});
		...
		if DB.getValue(v, "isactive", 0) == 1 then
			if (not bSkipGMOnly and User.isHost()) or (DB.getValue(v, "isgmonly", 0) == 0) then
				...
				for kComp,sEffectComp in ipairs(aEffectComps) do
					local vComp = EffectManagerSFRPG.parseEffectComp(sEffectComp);
					-- CHECK CONDITIONALS
					if vComp.type == "IF" then
						if not EffectManagerSFRPG.checkConditional(rActor, v, vComp.remainder) then
							break;
						end
