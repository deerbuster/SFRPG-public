...
function onInit()
	...
	ActionsManager.registerModHandler("spheal", modHeal);
	ActionsManager.registerResultHandler("spheal", onHeal);
end
...
function modHeal(rSource, rTarget, rRoll)
	...
	if rSource then
		...
		local nEffectCount;
		local aAddDice, nAddMod, nEffectCount = EffectManagerSFRPG.getEffectsBonus(rSource, {"HEAL"});
