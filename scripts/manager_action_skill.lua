...
function getRoll(rActor, sSkillName, nSkillMod, sSkillStat, sExtra)
	...
	if rActor and rActor.sType == "pc" then
		local sSkillLookup;
		local sSubSkill = nil;
		if sSkillName:match("^Profession") then
			sSubSkill = sSkillName:sub(12, -2);
			sSkillLookup = "Profession";
		else
			sSkillLookup = sSkillName;
		end
		local _,bUntrained = CharManager.getSkillValue(rActor, sSkillLookup, sSubSkill);
		if bUntrained then
...
function modSkill(rSource, rTarget, rRoll)
	...
	if rSource then
		...
		-- Get effects
		local aAddDice, nAddMod, nEffectCount = EffectManagerSFRPG.getEffectsBonus(rSource, {"SKILL"}, false, aSkillFilter);
		...
		if EffectManagerSFRPG.hasEffectCondition(rSource, "Blinded") then
		...
		elseif EffectManagerSFRPG.hasEffectCondition(rSource, "Dazzled") then
			if sSkillLower == "spot" or sSkillLower == "search" or sSkillLower == "perception" then
				bEffects = true;
				nAddMod = nAddMod - 1;
			end
		end
		if EffectManagerSFRPG.hasEffectCondition(rSource, "Fascinated") then
			if sSkillLower == "spot" or sSkillLower == "listen" or sSkillLower == "perception" then
				bEffects = true;
				nAddMod = nAddMod - 4;
			end
		end
		if EffectManagerSFRPG.hasEffectCondition(rSource, "Asleep") then
