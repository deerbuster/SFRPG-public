...
function delete()
	...
	if sClass ~= "charsheet" and sClass ~= "companionsheet" then
		wounds.setValue(0);
	end
...
function linkPCFields()
	...
	--Debug.chat("linkPCFields",nodeChar)
	if nodeChar then
		...
		rp.setLink(nodeChar.createChild("rp.current", "number"));
		sp.setLink(nodeChar.createChild("sp.total", "number"));		
		fatique.setLink(nodeChar.createChild("sp.fatique", "number"));
		

		eac.setLink(nodeChar.createChild("ac.totals.eac", "number"), true);
		kac.setLink(nodeChar.createChild("ac.totals.kac", "number"), true);
...
function linkCompanionFields()
	local nodeChar = link.getTargetDatabaseNode();
	--Debug.chat("linkCompanionFields",nodeChar)
	if nodeChar then
		name.setLink(nodeChar.createChild("name", "string"), true);
...
function setDefensiveVisible()
	...
	local nCMD = (kac.getValue() + 8);	
	
	defensiveicon.setVisible(v);
	
	eac.setVisible(v);
	eac_label.setVisible(v);
	kac.setVisible(v);
	kac_label.setVisible(v);
	cmd.setVisible(v);
	cmd.setValue(nCMD);
	...
	sr.setVisible(v);
	srlabel.setVisible(v);
	
	defensiveabilities.setVisible(v);
	defensiveabilitieslabel.setVisible(v);
