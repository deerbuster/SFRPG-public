...
function onHoverUpdate(x, y)
	...
	local nodeNPC = window.getDatabaseNode().getParent().getParent();	
	...
	if not parsed then
		parsed = true;
		rAttackRolls, rDamageRolls, rAttackCombos = CombatManager2.parseAttackLine(getActor(), getValue(), bRanged);
...
function getActor()
	...
	if ActorManager2.isCompanion(nodeActor) then
		actortype[1] = "pc";
	end
	
	--Debug.chat("string attackline getActor",actortype[1], nodeActor)
	return ActorManager2.getActor(actortype[1], nodeActor);
end
