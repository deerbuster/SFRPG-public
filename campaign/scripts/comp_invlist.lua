...
function onInit()
    ...
    DB.addHandler(DB.getPath(node, "*.weight"), "onUpdate", onEncumbranceChangedList);
    DB.addHandler(DB.getPath(node, "*.count"), "onUpdate", onEncumbranceChangedList);
    DB.addHandler(DB.getPath(node), "onChildDeleted", onEncumbranceChangedList);
end
...
function onClose()
    ...
    DB.removeHandler(DB.getPath(node, "*.weight"), "onUpdate", onEncumbranceChangedList);
    DB.removeHandler(DB.getPath(node, "*.count"), "onUpdate", onEncumbranceChangedList);
    DB.removeHandler(DB.getPath(node), "onChildDeleted", onEncumbranceChangedList);
end
...
function onEncumbranceChangedList()
...
function onDrop(x, y, draginfo)
    return ItemManager2.handleAnyDrop(window.getDatabaseNode(), draginfo);
end
