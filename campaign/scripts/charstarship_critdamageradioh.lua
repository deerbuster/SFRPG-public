...
function onInit()
	...
	initialize(bResource, labeltext, valuetext, width, tonumber(defaultval) or 1);
...
function initialize(bResource, sLabels, sValues, nOptionWidth, varDefault)
	...
	for k,v in ipairs(values) do
		...
		if labels[k] then
			...
			labelwidgets[k].setPosition("topleft", ((k-1)*option_width)+(w/2)+5, (h/2)+5);
		end
