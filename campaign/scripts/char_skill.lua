...
function onFreeSkill()	
	local sLabel = label.getValue();
	local t = DataCommon.skilldata[sLabel];
	if freeskill.getValue() ~= 0 then
		freewidget.setIcon("char_freeskill");
		freeranks.setVisible(true);
	else
		freewidget.setIcon(nil);
		freeranks.setVisible(false);		
	end	
end
