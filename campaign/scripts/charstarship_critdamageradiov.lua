...
function onInit()
	...
	initialize(bResource, labeltext, valuetext, height, tonumber(defaultval) or 1);
...
function initialize(bResource, sLabels, sValues, nOptionHeight, varDefault)
	...
	if sLabels then
		if bResource then
			local aStr = StringManager.split(sLabels, "|");
	...
	for k,v in ipairs(values) do
		...
		boxwidgets[k].setPosition("topleft", (w/2), ((k-1)*option_height)+5);
		
		if labels[k] then
			labelwidgets[k] = addTextWidget(widgetfont, labels[k]);
			w,h = labelwidgets[k].getSize();
			labelwidgets[k].setPosition("topleft", (w/2)+7, ((k-1)*option_height)+5);
		end
		
	end

	-- Set the width of the control
	setAnchoredWidth(#values * option_height);
